"""Gera rota urbana terrestre, mapas tematicos e relatorio executivo.

O calculo principal usa o servico Trip do OSRM, que otimiza a ordem das
paradas e devolve distancia, duracao, pernas e geometria sobre a malha viaria.
...
def read_csv(path: Path) -> list[dict[str, str]]:
    ...
    for encoding in ("utf-8-sig", "latin1", "cp1252"):
        ...
        raise ValueError(f"Nao foi possivel ler a codificacao de {path}")
...
def load_base(path: Path, requested_name: str | None) -> Point:
    ...
    for row in read_csv(path):
        ...
        if requested_name and name.lower().strip() == requested_name.lower().strip():
            return point
    if requested_name:
        raise ValueError(f"Farmacia base nao encontrada: {requested_name}")
    if not candidates:
        raise ValueError("Nenhuma farmacia de Belo Horizonte com coordenadas validas")
    return candidates[0]
...
def load_deliveries(path: Path, limit: int | None) -> list[Point]:
    ...
    for index, row in enumerate(read_csv(path), start=1):
        ...
        if limit and len(points) >= limit:
            break
    if not points:
        raise ValueError("Nenhum pedido com coordenadas validas")
    return points
...
def call_osrm_trip(points: list[Point], base_url: str, timeout: float) -> dict[str, Any]:
    ...
    if payload.get("code") != "Ok" or not payload.get("trips"):
        raise RuntimeError(f"OSRM nao retornou uma viagem valida: {payload}")
    return payload
...
def build_cached_dispatch_result(
    ...
    if not selected:
        raise ValueError("Cache OSRM sem farmacias")
    ...
    if not legs:
        raise ValueError("Cache OSRM sem rotas validas para os pedidos selecionados")
    ...
    return {
        "metadata": {
            ...
            "profile": "driving",
            "roundtrip": False,
            "route_model": "despachos individuais da farmacia para cada pedido",
...
def leaflet_html(result: dict[str, Any], style: dict[str, str]) -> str:
    ...
    route_label = "Rota urbana circular otimizada" if result["metadata"].get("roundtrip") else "Plano de despachos urbanos"
    return f"""<!doctype html>
<html lang=\"pt-BR\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width\">
<title>{style['name']} - rota terrestre</title>
...
def render_report(result: dict[str, Any], output_path: Path) -> None:
    try:
        ...
        raise RuntimeError("O relatorio PDF requer matplotlib") from exc
    ...
    with PdfPages(output_path) as pdf:
        ...
        route_title = "Rota otimizada sobre a malha viaria" if result["metadata"].get("roundtrip") else "Despachos terrestres calculados sobre a malha viaria"
        ...
        indicators = [
            ...
            ("Tempo em deslocamento", f"{summary['driving_minutes']:.2f} min"),
            ("Tempo de atendimento", f"{summary['service_minutes']:.2f} min"),
            ("Velocidade media calculada", f"{summary['average_speed_kmh']:.2f} km/h"),
        ...
        fig.text(.08, .085, "Combustivel, custo e CO2 sao estimativas parametrizadas; congestionamento em tempo real nao esta incluido.", fontsize=8)
        pdf.savefig(fig)
        plt.close(fig)
        ...
        for page_start in range(0, len(legs), rows_per_page):
            ...
            ax.axis("off")
            fig.suptitle("Detalhamento dos trechos da rota", fontsize=16, fontweight="bold", y=.96)
            ...
            fig.text(.07, .15, "Os tempos sao estimativas do motor de roteamento e podem variar com transito, obras e restricoes temporarias.", fontsize=8)
            pdf.savefig(fig)
            plt.close(fig)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Gera rota e relatorio de transporte urbano terrestre")
...
def main() -> None:
    ...
    try:
        if args.usar_cache:
            raise requests.ConnectionError("modo de cache solicitado")
        else:
    ...
    print("[3/5] Gerando quatro mapas interativos...")
    write_html_maps(result, args.saida)
    print("[4/5] Gerando relatorio PDF...")
